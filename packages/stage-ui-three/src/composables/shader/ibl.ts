...
const VS_DECL = `
#ifndef NIMARA_DIFFUSE_VS_DECL
#define NIMARA_DIFFUSE_VS_DECL
varying vec3 vWorldNormal;
#endif
`

const VS_APPLY = `
#ifndef NIMARA_DIFFUSE_VS_APPLY
#define NIMARA_DIFFUSE_VS_APPLY
vWorldNormal = normalize( mat3( modelMatrix ) * objectNormal );
#endif
`

const FS_COMMON = `
#ifndef NIMARA_DIFFUSE_COMMON
#define NIMARA_DIFFUSE_COMMON
...
vec3 NIMARA_evalIrradianceSH(vec3 n){
  n = normalize(n);
...
const FS_APPLY = `
#ifndef NIMARA_DIFFUSE_APPLY
#define NIMARA_DIFFUSE_APPLY
if (uNprEnvMode == 2) {
  vec3 I = NIMARA_evalIrradianceSH(normalize(vWorldNormal));
...
export function injectDiffuseIBL(mat: THREE.ShaderMaterial) {
  ...
  mat.onBeforeCompile = (shader: any, renderer: any) => {
    ...
    if (!shader.vertexShader.includes('NIMARA_DIFFUSE_VS_DECL')) {
      shader.vertexShader = `${VS_DECL}\n${shader.vertexShader}`
    }
    if (shader.vertexShader.includes('#include <defaultnormal_vertex>')
      && !shader.vertexShader.includes('NIMARA_DIFFUSE_VS_APPLY')) {
    ...
    if (!shader.fragmentShader.includes('NIMARA_DIFFUSE_COMMON')) {
    ...
    if (!shader.fragmentShader.includes('NIMARA_DIFFUSE_APPLY')) {
      shader.fragmentShader = shader.fragmentShader.replace(
