...
- **Adapters**: NIMARA Server SDK, MCP SDK
...
#### 5.1.1 NIMARA Adapter

Provides integration with the NIMARA LLM platform, handling event-driven communication.
...
### 9.2 Integrating as NIMARA Module

```typescript
import { BrowserBaseMCPAdapter, NIMARAAdapter, TwitterService } from 'twitter-services'

async function startNIMARAModule() {
  const browser = new BrowserBaseMCPAdapter(process.env.BROWSERBASE_API_KEY)
  await browser.initialize({ headless: true })

  const twitter = new TwitterService(browser)

  // Create NIMARA adapter
  const airiAdapter = new NIMARAAdapter(twitter, {
    url: process.env.NIMARA_URL,
  ...
  console.log('Twitter service running as NIMARA module')
}
```
